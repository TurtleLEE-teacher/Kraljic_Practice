...
export default function ScenarioBriefing({ background, quadrant }: ScenarioBriefingProps) {
  ...
  return (
    <div className={`rounded-xl border-2 ${accent.border} ${accent.bg} p-5 space-y-4`}>
      {/* Header */}
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="text-lg font-bold text-gray-900">{background.companyName}</h2>
          <p className="text-xs text-gray-500">{background.industry}</p>
        </div>
        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${accent.badge} ${accent.badgeText}`}>
          {quadrant.nameKo}
        </span>
      </div>
      ...
      <div className="bg-white/70 rounded-lg p-3">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h3 className="text-sm font-semibold text-gray-900">{background.itemName}</h3>
        </div>
        <p className="text-xs text-gray-600 leading-relaxed">{background.itemDescription}</p>
      ...
      <ul className="space-y-1">
        {background.keyMetrics.map((metric, index) => (
          <li key={index} className="flex items-start gap-2 text-xs text-gray-600">
            <span className={`mt-0.5 w-1.5 h-1.5 rounded-full flex-shrink-0 ${accent.icon.replace('text-', 'bg-')}`} />
            {metric}
          </li>
        ))}
      </ul>
